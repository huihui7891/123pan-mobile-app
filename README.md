...
- **多选批量操作**：整理模式下新增"复制"按钮，支持多选文件/文件夹批量复制；文件夹支持递归复制（创建同名文件夹 → 遍历内容 → 逐个复制 → 子文件夹递归），支持文件夹+文件混合复制
...
- **包名与签名更新（v1.0.113）**：包名从 `com.pan.mobile` 改为 `com.yunpan.mobile`，更换为新的签名密钥，移除 `REQUEST_INSTALL_PACKAGES` 权限以降低杀毒软件误报
...
- 移除了相册视图功能（全盘扫描图片/视频存在重复显示、扫描慢等问题，已移除）
